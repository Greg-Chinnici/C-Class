...
bool Game::isCloseToTarget(int iRow, int iColumn)
{
	...
	// the target boat. 
	// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
	if ((std::abs(iRow - m_iRowBoat) <= 1) && (std::abs(iColumn - m_iColumnBoat) <= 1))
	{
		bClose = true;
	}
...
void Game::showSunk()
{
    ...
    a_cGrid[this->m_iRowBoat][this->m_iColumnBoat] = 'X';
	return;
}
...
std::tuple<int, int> Game::promptForGridCoord()
{
	...
	/* ?? Use std::cin ?? */
    std::cout<< "What row do you want to fire at?: " << std::endl;
    std::cin >> iRow;
        /* ?? Prompt user using std::cout for column ?? */	
	/* ?? Use std::cin ?? */
    std::cout<< "What column do you want to fire at?: " << std::endl;
    std::cin >> iColumn;
